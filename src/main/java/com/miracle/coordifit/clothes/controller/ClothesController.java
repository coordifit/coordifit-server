...
public class ClothesController {
        ...
        try {
            if (req.getItems() != null && !req.getItems().isEmpty()) {
                validateCategoryCodes(
                    req.getItems().stream().map(ClothesCreateWithImagesRequest::getCategoryCode).collect(Collectors.toList())
                );
            }
    ...
    public ApiResponseDto<Void> update(
        ...
        try {
            if (req.getCategoryCode() != null) validateCategoryCode(req.getCategoryCode());
    ...
    public ApiResponseDto<Map<String, Object>> getClothes(
        ...
        if ("price".equalsIgnoreCase(sort)) {
            ...
            comp = Comparator.comparing(dto -> dto.getName() == null ? "" : dto.getName(), String.CASE_INSENSITIVE_ORDER);
        } else {
            comp = Comparator.comparing(ClothesDetailDto::getPurchaseDate, Comparator.nullsLast(Comparator.naturalOrder()));
        }
    ...
    private void validateCategoryCode(String categoryCode) {
        ...
        if (code.isEmpty()) throw new IllegalArgumentException("유효하지 않은 categoryCode(공백)");
        int n = clothesRepository.existsActiveCategoryCount(code);
        if (n == 0) throw new IllegalArgumentException("유효하지 않은 categoryCode: " + code);
    }
